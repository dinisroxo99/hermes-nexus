# Roundtable Review — 28_KNOWLEDGE_PRECEDENCE_AND_DRIFT.md
...
| Expert | Assessment |
|---|---|
| **Distributed Systems Architect** | `Knowledge Precedence and Drift Rules` fits the overall boundary, but must continue to name its owner, inputs and failure behavior rather than becoming an implicit shared state machine. |
...
| **Application Security / Trust Engineer** | Requires provenance, secret redaction and poisoning controls before data from `Knowledge Precedence and Drift Rules` is reused for Project Expert answers or training. |
| **SRE / Observability Engineer** | `Knowledge Precedence and Drift Rules` is operationally acceptable if its derived state can be rebuilt and its dependencies expose health/readiness and bounded timeouts. |
| **Git / SCM & Developer Tooling Engineer** | Requires every code-related historical claim in `Knowledge Precedence and Drift Rules` to preserve the repository revision that produced it. |
...
| **API / MCP Integration Engineer** | `Knowledge Precedence and Drift Rules` should refer to stable domain contracts and avoid leaking provider-specific schemas into Project Map core logic. |
| **Developer Experience / Product Engineer** | `Knowledge Precedence and Drift Rules` is useful as long as operators can inspect the reason behind decisions (impact, scope, conflict, routing) instead of receiving opaque automation. |
| **QA / Verification Engineer** | `Knowledge Precedence and Drift Rules` should define acceptance criteria that can be asserted without trusting an LLM's own claim of success; evidence and revision linkage remain mandatory. |

## Consensus

Keep; add expiry rules for non-Git facts and a persistent conflict registry.
...
- Made unresolved conflicts queryable.
