# Roundtable Review — 26_PROJECT_EXPERT_DATA_PIPELINE.md
...
| Expert | Assessment |
|---|---|
| **Distributed Systems Architect** | `Project Expert Data Pipeline` fits the overall boundary, but must continue to name its owner, inputs and failure behavior rather than becoming an implicit shared state machine. |
...
| **Application Security / Trust Engineer** | Requires provenance, secret redaction and poisoning controls before data from `Project Expert Data Pipeline` is reused for Project Expert answers or training. |
...
| **Git / SCM & Developer Tooling Engineer** | Requires every code-related historical claim in `Project Expert Data Pipeline` to preserve the repository revision that produced it. |
...
| **API / MCP Integration Engineer** | `Project Expert Data Pipeline` should refer to stable domain contracts and avoid leaking provider-specific schemas into Project Map core logic. |
| **Developer Experience / Product Engineer** | `Project Expert Data Pipeline` is useful as long as operators can inspect the reason behind decisions (impact, scope, conflict, routing) instead of receiving opaque automation. |
| **QA / Verification Engineer** | `Project Expert Data Pipeline` should define acceptance criteria that can be asserted without trusting an LLM's own claim of success; evidence and revision linkage remain mandatory. |

## Consensus

Keep; strengthen poisoning filters and validation before training.
...
- Excluded stale/rejected/unlinked runs.
- Required evidence/acceptance for strong training targets.
